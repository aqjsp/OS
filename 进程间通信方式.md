...
进程间通信（IPC，InterProcess Communication）是指在不同进程之间传播或交换信息。IPC的方式通常有管道（包括无名管道和命名管道）、消息队列、信号量、共享内存、Socket（套接字）等。其中 Socket 还支持不同主机上的两个进程通信。

五种经典 System V / POSIX IPC 可以先按「要不要拷数据、要不要同步」排一下：管道和消息队列都走内核、两次拷贝；共享内存只在建立映射时进内核，之后用户态直接读写，所以最快，但必须另配信号量或 futex。Socket 不在下面五节展开，但它是跨机 IPC 的默认选项。

![五种 IPC：管道到共享内存，拷贝次数递减、同步责任递增](./image/ipc-compare.svg)
